				...
				if (!empty($sql_labTemp)) {
					while ($get_labTemp = mysqli_fetch_assoc($sql_labTemp)) {
						...
						if (mysqli_num_rows($exist) > 1) {
							...
							$insertRes = mysqli_query($db, "UPDATE  patient_test_result SET app_id = ".$app_id.",test_id = ".$form.",ref_id = '".$ref."', value = '".$val."', test_name = ".$temp.", lab_temp_id = ".$id." WHERE ref_id LIKE '".$ref."' AND lab_temp_id = ".$ext['lab_test_id']."");
						}
						if ($insertRes) {
							$success = "Done2";
							$update_await = mysqli_query($db,"UPDATE patient_test_group SET awaiting_result = 1,lab_id = ".$user_id." WHERE link_ref = '".$ref."'");
							//Notify Doctor
			...
			while ($get_labtn = mysqli_fetch_assoc($sql_labtn)) {
				...
				if (!empty($sql_labTemp)) {
					while ($get_labTemp = mysqli_fetch_assoc($sql_labTemp)) {
						...
						if ($insertRes) {
							$success = "Done";
							$update_await = mysqli_query($db,"UPDATE patient_test_group SET awaiting_result = 1,lab_id = ".$user_id." WHERE ref_id LIKE '".$ref."' AND front_desk = '".$front_desk."' AND patient_appointment_id = 0");
							//Notify Doctor
							$doc1 = mysqli_query($db, "SELECT doctor_id,patient_id FROM patient_test_group WHERE link_ref LIKE '".$ref."'");
							$doc2 = mysqli_fetch_assoc($doc1);
							$doc = $doc2['doctor_id'];
            	...
            	<div class="row">
            		...
            		<hr>
							...
							foreach ($p_name as $pname) {
								$pan = $pname['surname']." ".$pname['first_name']." ".$pname['middle_name'];
								$page = $pname['age'];
								$psex = $pname['sex'];
							}
							mysqli_query($db, "INSERT INTO notifications(staff_id,patient_id,message,link,time_taken,status)  
							VALUES (".$doc.",".$p_id.",'Lab Results Are Available For: ','view_test?id=".$_GET['id']."&pid=".$p_id."&n=".$pan."&a=".$page."&s=".$psex."&did=".$user_id."',NOW(),0)");
