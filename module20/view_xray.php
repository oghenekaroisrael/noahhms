 ...
 <?php include 'inc/main_header.php';?>
        ...
        <div class="content">
            <div class="container-fluid">
                <div class="row">
				 <?php
                      $noarray = database::getInstance()->select_xray_to($value);
				...
				<table class="table table-bordered">
					...
					<tr>
						<td colspan="4"><b>Selected Xrays:  </b>
							<?php
								$sql_al = mysqli_query($db, "SELECT name, type FROM xray_requests WHERE link = '".$value."'");
								while ($get_al = mysqli_fetch_assoc($sql_al)) {
									$al_lti = $get_al['name'];
								if (!empty($al_lti)) {
									$sql_lt = mysqli_query($db, "SELECT name FROM xray WHERE id = ".$al_lti."");
				...
				<table  class="table table-bordered">
					<?php
						$sql = mysqli_query($db, "SELECT * FROM patient_xray_result WHERE patient_id = ".$_GET['pid']."  AND ref_id = '".$_GET['id']."' ORDER BY  id DESC");
