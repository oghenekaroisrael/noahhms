...
<script type="text/javascript">
	...
	a(function () {
		...
		a('#case').on('submit', function (e) {
			...
			a.ajax({
				type: "POST",
				data: a('#case').serialize()+ '&p_id=' + p_id + '&doc_id=' +doc_id + '&id=' + value + '&ins=newCaseD&req='+req,
				url: "../func/verify.php",
        ...
        a('#formed').submit(function(e){
			...
			s.ajax({
				type: "POST",
				data: a('#formed').serialize() + "&val=" + pat + "&doc=" + doc + "&app=" + id + "&ins=newTestRequest",
				url: "../func/verify.php",
				success: function(res) {
					if(res == "Done"){
					window.location = "request_test.php?id=<?php echo $_GET['id'] ?>&stat=success";
					}else{
						document.getElementById("load").style.display = "none";
        ...
        a('#drug_list').on('submit', function (e) {
			...
			a.ajax({
				...
				success: function(res) {
					a("#get_result").html(res).fadeIn("slow");
					console.log(res);
					window.location = "request_test.php?id=<?php echo $_GET['id'] ?>&progress=true";
				}
			});
        });
