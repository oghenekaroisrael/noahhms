<?php 
	...
	$value = $_GET['ref'];
	$p_id = $_GET['id'];
	$disease = database::getInstance()->get_name_from_id('disease','treatments','id', $value);
	$reference = database::getInstance()->get_name_from_id('reference','treatments','id', $value);
	$diagnosis = database::getInstance()->get_name_from_id("name","treatment_list","id",$disease);
		if ($reference != "") {
				$_SESSION['presc'] = $reference;
		}else	if (!isset($_SESSION['presc'])) {
									...
									<div class="content table-responsive table-full-width">
										<table class="table table-hover table-striped">
											...
											<tbody>
												<?php 
			                                    	$coun =0;
			                                    		$tmp_presc = database::getInstance()->select_from_where2_DESC('temp_presct','pid',$_SESSION['presc']);
...
<script type="text/javascript">
	...
	a(function () {
		...
		a('#case').on('submit', function (e) {
			...
			a.ajax({
				type: "POST",
				data: a('#case').serialize()+ '&p_id=' + p_id + '&doc_id=' +doc_id + '&id=' + value + '&ins=newCaseD1',
				url: "../func/verify.php",
        ...
        a('#drug_list').on('submit', function (e) {
			...
			a.ajax({
				type: "POST",
				data: a('#drug_list').serialize()+ '&p_id=' + p_id +'&pre='+ did + '&doc_id=' +doc_id + '&id=' + value + '&ins=newCaseDrug1',
				url: "../func/verify.php",
				success: function(res) {
					a("#get_result").html(res).fadeIn("slow");
					console.log(res);
					window.location = "patient_details.php?id=<?php echo $_GET['id'] ?>&ref=<?php echo $_GET['ref']; ?>";
				}
			});
        });
