...
<!--Additional files-->
		 ...
		 <div class="content no-print">
                            	<div class="row">
									 <div class="col-md-12">
									  <h3 class="text-center">Scans Results</h3>
				                        <div class="card">
				                                ...
				                                <table id="pro"class="table table-hover table-striped">
				                                    ...
				                                    <tbody>
													 <?php
															...
															$sql = mysqli_query($db, "SELECT * FROM patient_xray_result WHERE patient_id = ".$_GET['pid']."  AND ref_id = '".$_GET['ref']."' ORDER BY  id DESC");
															$sql1 = mysqli_query($db, "SELECT * FROM patient_scan_result WHERE patient_id = ".$_GET['pid']."  AND ref_id = '".$_GET['ref']."' ORDER BY  id DESC");
															if ($sql) {
														...
														<td width="10%">
																<div class="btn-group">
																	...
																	<ul class="dropdown-menu" role="menu">
																		...
																		<li class="divider"></li>
																		<li><a onclick="sure(<?php echo $get['id']; ?>,'<?php echo $get['file']; ?>')">Delete</a></li>
																	</ul>
																</div>
															</td>
														...
														if ($sql1) {
															//here
															while ($get = mysqli_fetch_assoc($sql1)) {
															?>
															<tr>
														...
														<td width="10%">
																<div class="btn-group">
																	...
																	<ul class="dropdown-menu" role="menu">
																		<li><a onclick="download<?php echo $get['id']; ?>()">Download File</a></li>
																		<li class="divider"></li>
																		<li><a onclick="sure2(<?php echo $get['id']; ?>,'<?php echo $get['file']; ?>')">Delete</a></li>
																	</ul>
																</div>
															</td>
														<script type="text/javascript">
										...
										<a href='upload_doc?id=<?php echo $value;?>&pid=<?php echo $pid; ?>&ref=<?php echo $ref; ?>' class="btn btn-info btn-fill pull-right">Upload Scans</a>
				                    </div>
		...
		var val = ID;
		 document.getElementById("load").style.display = "block";
          s.ajax({
            type: 'post',
            url: '../func/del.php',
            data: "val=" + val +  '&ins=delXrayFile',
             success: function(data)
            {
				document.getElementById("load").style.display = "block";
				if (data === 'Done') {
					console.log(data);
						window.location = 'lab_results?id=<?php echo $value; ?>&pid=<?php $_GET['pid'];?>&ref=<?php $_GET['ref']; ?>';
				  }
				  else {
