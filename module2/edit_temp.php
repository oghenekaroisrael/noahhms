	                                       ...
	                                       <div class="col-md-12">
	                                            <div class="form-group">
	                                                <label>Template Name</label>
	                                                 <input type="text" class="form-control" name="temp_name" value="<?php echo $temp_name;?>" />
	                                            </div>
	                                        </div>
										</div>
									</div>
									<div class="clearfix"></div>
									<div id="get_resultop"></div>
...
<script type="text/javascript">
	var a=jQuery .noConflict();			
	a(function () {
		a('#editTempaName').on('submit', function (e) {
			e.preventDefault();
			document.getElementById("load").style.display = "block";
			var val = "<?php echo $value; ?>";
			a.ajax({
				type: "POST",
				data: a('#editTempaName').serialize() + "&val=" + val + '&ins=editTempa',
				url: "../func/edit.php",
				success: function(res) {
					document.getElementById("load").style.display = "none";
					a("#get_resultop").html(res).fadeIn("slow");
				}
			});
        });
    });
