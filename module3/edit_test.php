                                ...
                                <form id="edit_sche">
									...
									<div class="col-md-12">
										<div class="row">
	                                       <div class="col-md-12">
	                                            <div class="form-group">
	                                                <label>Template To Be Used</label>
	                                                <select class="form-control" id="type" name="template">
														<option value="" class="disabled">Select A Template To Be Used For This Test</option>

														<?php
																if (!empty($ow['template']) AND $ow['template'] != 0) {?>
																	<option selected="selected" value="<?php echo $ow['template']?>"><?php 
														$db = mysqli_connect("localhost","root","","noahhms");
														$sql_c = mysqli_query($db, "SELECT name FROM lab_temp_name WHERE id = ".$ow['template']."");
														$get_c = mysqli_fetch_assoc($sql_c);
														echo $get_c['name'];
														?>															
														</option>
														<?php
																}else{
																	echo "No Template has Been Chosen";
																}
														?>
														<?php 
														$contt = Database::getInstance()->select_from_where_no('lab_temp_name','id',$ow['template']);
														foreach($contt as $row){
														?>
